...
async def merge_pdfs_endpoint(
    ...
    pid_file: Optional[UploadFile] = File(None, description="P&ID PDF file"),
    misc_file: Optional[UploadFile] = File(None, description="Misc PDF file"),
    ...
    Emails result to customer and admin.
    """
    ...
    try:
        ...
        if pid_file:
            try:
                pid_content = await pid_file.read()
                pid_reader = PdfReader(io.BytesIO(pid_content))
        ...
        if misc_file:
            try:
                misc_content = await misc_file.read()
                misc_reader = PdfReader(io.BytesIO(misc_content))
